...
**简构 (Jiǎn Gòu)​ 框架**是一个适合小公司和个人开发者的 Vue 3 后台管理框架，含 Spring Boot 4 单体架构后端通用模板。
...
如需使用本框架作为项目模板，请克隆[版本发布仓库](https://github.com/footprintcat/admin-template-release.git)以获取干净的代码：

```bash
git clone https://github.com/footprintcat/admin-template-release.git
```

版本发布仓库包含经过清理的代码，适合直接用于开发新项目。
...
1. <b>创建 template 分支方式（推荐，适用于开发正式项目）</b>
    - <b>使用模板时，</b>在你的代码仓库中添加一个 `template` 分支，这个分支的代码与[版本发布仓库](https://github.com/footprintcat/admin-template-release.git)的代码保持一致，然后在 `main` 主分支（或其他分支上）进行开发。
...
2. <b>添加 `template` 远端方式（适合希望随时能够更新框架的情况，这种情况 git 历史中会保留[版本发布仓库](https://github.com/footprintcat/admin-template-release.git)的所有提交记录）</b>
    - <b>使用模板时，</b>因为 Git 支持添加多个远端（remote），我们可以直接在本地创建一个新的 Git 仓库，然后将[版本发布仓库](https://github.com/footprintcat/admin-template-release.git)添加为 `template` 远端。拉取代码后，你可以基于 `template/main` 创建一个 `main` 分支，然后推送到你自己的远端（如 `origin/main`）。后续你可以在 `main` 分支上进行开发。
    - <b>需要更新时，</b>先 `git fetch template` 把框架最新提交拉取下来，然后在你的 `main` 分支上 `git merge template/main` 分支即可。
3. 如果您对 Git 比较熟悉，你也可以采用其他的方式来跟随模板代码库的更新。比如可以利用代码平台（GitHub, Gitee, 或自建的 GitLab 等）的 Pull Request 或 Actions 等来操作，不过比较复杂，不建议小白采用这种方式。
